...
eachocc(x) = (p for p in eachelem(spbasis(x)) if isocc(x, p))
eachunocc(x) = (p for p in eachelem(spbasis(x)) if isunocc(x, p))
...
eachunocc(ref::RefStates.Vacuum) = eachelem(spbasis(ref))
