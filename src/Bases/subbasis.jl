...
Base.in(x, a::_Arrayish{<:Sub}) = convert(superbasis(eltype(a)), x) in eachelem(SB)

let IXMAPS=Dict{Any, Any}()
    global subindexmap
    function subindexmap(SB::Type{<:Sub})
        if !haskey(IXMAPS, SB)
            IXMAPS[SB] = [index(b) for b in eachelem(supbasis(SB)) if b in eachelem(SB)]
        end
